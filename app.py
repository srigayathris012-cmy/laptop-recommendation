...
# ================== GEMINI SIDEBAR ==================
# Load Gemini logo from URL
logo_url = "https://upload.wikimedia.org/wikipedia/commons/thumb/0/05/Google_Chat_icon_2023.svg/64px-Google_Chat_icon_2023.svg.png"
response = requests.get(logo_url)
gemini_logo = Image.open(BytesIO(response.content))
st.sidebar.image(gemini_logo, width=50)
st.sidebar.title("🤖 Ask Gemini AI")

# Initialize chat history
if "messages" not in st.session_state:
    st.session_state.messages = []

user_question = st.sidebar.text_input("Ask me anything about laptops:")

def ai_answer(question):
    q = str(question).lower()
    # First check dataset based AI
    ...
    if "gaming" in q:
        ...
        # Call Gemini API for anything outside dataset rules
        try:
            completion = genai.chat.create(
                model="models/text-bison-001",
                messages=[{"role":"user","content":question}]
            )
            response = completion.last.split("\n")[0]
        except:
            response = "Sorry, Gemini could not answer that."
    return response

# Update chat history
if user_question:
    st.session_state.messages.append({"role":"user","content":user_question})
    st.session_state.messages.append({"role":"assistant","content":ai_answer(user_question)})
